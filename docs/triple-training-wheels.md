...
- **Web demo.** Python-backed sim: `WARM=1 bash scripts/mppi-server.sh`, then open `/#triple`. The server steps the training plant with the teacher in the loop; the browser only renders and sends inputs. The sim runs at ~30% real time at 4096 samples.
