...
- **Web demo.** Python-backed sim: `WARM=1 bash scripts/mppi-server.sh`, then open `/#triple`. The server steps the training plant with the teacher in the loop; the browser only renders and sends inputs. It runs in real time at 4096 samples: the rollout is a fused multicore numba kernel (`train/mppi/fast_rollout.py`, ~12 ms per replan on an M2 Pro, parity-tested against the torch path).
