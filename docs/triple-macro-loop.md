...
Last updated: 2026-09-19 ~23:42 CT  
...
- **Live (~23:42 CT):** VM `cartpole-train-od` RUNNING us-east1-b (L4 ~98%/11.4GB). TB http://34.148.138.48:6006/
  - **A** PPO swing f50 ~**u220**/400 nt_at_goal/UUU~**0.037** align_UUU~−0.055 — still flat on hold; leave alone (no mid-kill)
  - **B** **TQC UUU live** (`train_triple_tqc.py` pid 133757) since **03:37Z / ~22:37 CT** — run `tqc-uuu-f40-hold-wide_2`; ~**125.5k**/300k steps (ckpt @125k), `ep_rew_mean` −258→**+541** (still climbing; +106 since 93.5k / +239 since 61k), success_rate **0**, ent_coef~0.012; eval@125k mean_rew~**569** (534→517→543→563→569) success 0. **Not flat** — keep metering to ~150k (next eval ~150k)
  - **C** PPO combo f40 ~**u230**/400 nt_at_goal/UUU~**0.042** align_UUU~0.20 ent~3.4 — leave alone
- **Next micro-task:** **meter TQC only.** Track B timesteps / `ep_rew_mean` / success / eval@150k. Leave A/C alone. If TQC still flat after ~**150k** steps (reward plateau + success 0) → stage two-policy handoff (do not spam entropy knobs). If real UUU hold lift (success>0 or hold eval) → stage next EP specialist. No mid-kill.
