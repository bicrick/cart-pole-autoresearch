#!/usr/bin/env bash
# Keep nowalls training alive: wait → restart → loop (never exit while armed).
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p logs
find_pid() { ps -eo pid=,args= | awk '/^[ ]*[0-9]+[ ]+python3 train\/train\.py .*ft-e16384-r256-nowalls-oob20-fold55/{print $1; exit}'; }
start_train() {
  nohup python3 train/train.py \
    ...
    --logdir runs --run-name ft-e16384-r256-nowalls-oob20-fold55 \
    --checkpoint policies/checkpoint-nowalls.pt --out policies/policy-nowalls.json \
    >> logs/train-nowalls.log 2>&1 &
...
while true; do
  ...
  if [[ -n "$(find_pid || true)" ]]; then
    echo "$(date -u +%FT%TZ) nowalls already running; continue wait" >> logs/continue-nowalls.log
    continue
  fi
  start_train
  sleep 5
