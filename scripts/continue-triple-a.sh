...
# Cold wipe via .triple-a-s1-antidd-v2 (anti-DDD after hang-farm + NaN @u194–220).
# 2026-09-20: LR 3e-4→1e-4 after NaN @u228; then W_DOWN=0 UU_BIAS=2 after DDD farm+NaN log_std.
set -euo pipefail
cd "$(dirname "$0")/.."
...
while true; do
  ...
  if [[ -n "${PID}" ]]; then
    ...
    echo "$(date -u +%FT%TZ) triple-a exited; restarting as S1 walls-swing anti-DDD" >> logs/continue-triple-a.log
    sleep 5
  fi
  ...
  if [[ "${TRIPLE_A_COLD:-1}" == "1" && ! -f policies/.triple-a-s1-antidd-v2 ]]; then
    rm -f policies/checkpoint-triple-a.pt
    touch policies/.triple-a-s1-antidd-v2
    echo "$(date -u +%FT%TZ) cold-start triple-a S1 anti-DDD; marker .triple-a-s1-antidd-v2 set" >> logs/continue-triple-a.log
  fi
  # S1: walls-on, hang/bottom heavy. Anti-DDD: W_DOWN=0 UU_BIAS=2 ENERGY_W=0.6 LR=1e-4.
  nohup env TRACK_WALLS=1 NUM_ENVS="${NUM_ENVS:-8192}" FORCE_LIMIT=40 \
    ...
    NEAR_GOAL_P=0.10 WRONG_EQ_P=0.0 UU_BIAS=2.0 ANNEAL_UPDATES=0 \
    GOAL_SWITCH_P=0.0 FOLD_PAIR_P=0.0 \
    CART_BARRIER_COEF=10 W_UP=5.0 W_DOWN=0.0 ALPHA_TH=0.5 \
    FALL_GRACE_STEPS=20 START_GRACE_STEPS=40 \
    INIT_MODE=bottom INIT_NOISE=0.3 ENERGY_W=0.6 LR=1e-4 ENT=0.05 \
    RUN_NAME=ft-triple-a-e8192-r256-uuu-walls-swing-s1-f40-hang085-antidd-wd0-uub2-ew06-ent05-lr1e4 \
  ...
  echo "$(date -u +%FT%TZ) STARTED triple-a S1 anti-DDD W_DOWN=0 UU_BIAS=2 pid=$!" >> logs/continue-triple-a.log
  sleep 30
done
