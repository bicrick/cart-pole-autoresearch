...
while true; do
  ...
  # S1: walls-on, hang/bottom heavy, progress on, product reward. Deliver toward UUU.
  nohup env TRACK_WALLS=1 NUM_ENVS="${NUM_ENVS:-8192}" FORCE_LIMIT=40 \
    ...
    INIT_MODE=bottom INIT_NOISE=0.3 ENERGY_W=0.5 LR=3e-4 ENT=0.05 \
    RUN_NAME=ft-triple-a-e8192-r256-uuu-walls-swing-s1-f40-hang085-prog1-flip-ent05-lr3e4 \
  ...
  echo "$(date -u +%FT%TZ) STARTED triple-a S1 walls-swing pid=$!" >> logs/continue-triple-a.log
  sleep 30
done
