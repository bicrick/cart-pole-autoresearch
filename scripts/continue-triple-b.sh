...
# progress+product, flip-augment, forceLimit=40, soft barrier. Cold wipe via v5 marker.
set -euo pipefail
cd "$(dirname "$0")/.."
...
while true; do
  ...
  if [[ "${TRIPLE_B_COLD:-1}" == "1" && ! -f policies/.triple-b-hold-progress-flip-v5 ]]; then
    rm -f policies/checkpoint-triple-b.pt
    touch policies/.triple-b-hold-progress-flip-v5
    echo "$(date -u +%FT%TZ) cold-start triple-b (UUU hold near_target / f40 / progress+flip); marker set" >> logs/continue-triple-b.log
  fi
  # Slot B: UUU hold/stabilize — near_target high, low hang, progress+product.
  nohup env NUM_ENVS="${NUM_ENVS:-8192}" FORCE_LIMIT=40 \
    ...
    INIT_MODE=near_target ENERGY_W=0.15 LR=3e-4 \
    RUN_NAME=ft-triple-b-e8192-r256-uuu-hold-f40-bar10-prog1-flip \
