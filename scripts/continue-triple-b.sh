...
start_ppo_h1() {
  ...
  if [[ -f policies/.triple-b-h1-atrpo-avc-v1 ]]; then
    ...
    if [[ ! -f policies/.triple-b-h1-atrpo-avc-cold-v1 ]]; then
      rm -f policies/checkpoint-triple-b.pt
      touch policies/.triple-b-h1-atrpo-avc-cold-v1
      echo "$(date -u +%FT%TZ) ATRPO+AVC cold wipe collapsed ATRPO ckpt; cold marker set" >> logs/continue-triple-b.log
    fi
    nohup env TRACK_WALLS=1 NUM_ENVS="${NUM_ENVS:-8192}" FORCE_LIMIT=40 \
      ...
      USE_SDE=0 AVG_REWARD=1 AVC_NU=0.2 EPISODE_LEN=1200 \
      RUN_NAME=ft-triple-b-e8192-r256-uuu-walls-hold-h1-f40-nt1-in005-rpo01-era05-ew035-atrpo-avc02-ent0-bar10-prog1-flip \
    ...
    echo "$(date -u +%FT%TZ) STARTED triple-b H1 ATRPO+AVC ν=0.2 + ENERGY_W=0.35 RPO+ERA ENT=0 pid=$!" >> logs/continue-triple-b.log
  elif [[ -f policies/.triple-b-h1-atrpo-v1 ]]; then
