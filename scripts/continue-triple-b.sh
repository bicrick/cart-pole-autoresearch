...
# v6 cool-ent: LR=1e-4, ENT=0.03, f40, soft barrier, progress+flip.
set -euo pipefail
cd "$(dirname "$0")/.."
...
while true; do
  ...
  if [[ "${TRIPLE_B_COLD:-1}" == "1" && ! -f policies/.triple-b-cool-ent-v6 ]]; then
    rm -f policies/checkpoint-triple-b.pt
    touch policies/.triple-b-cool-ent-v6
    echo "$(date -u +%FT%TZ) cold-start triple-b (cool-ent v6 / f40 / ent0.03 / lr1e-4); marker set" >> logs/continue-triple-b.log
  fi
  nohup env NUM_ENVS="${NUM_ENVS:-8192}" FORCE_LIMIT=40 \
    ...
    INIT_MODE=near_target ENERGY_W=0.15 LR=1e-4 ENT=0.03 \
    RUN_NAME=ft-triple-b-e8192-r256-uuu-hold-f40-bar10-prog1-flip-ent03-lr1e4 \
